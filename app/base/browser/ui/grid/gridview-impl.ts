...
import { LayoutPriority, SplitView, Sizing, Split } from "base/browser/ui/splitview/splitview";
import { Orientation, Sash } from "base/browser/ui/sash/sash";
import { tail2 } from "base/common/arrays";
import { IBranchNode, ILeafNode, GridNode, GridBranchNode, IView, Box, IGridViewStyles, orthogonal, IGridViewOptions, IGridView } from "base/browser/ui/grid/gridview";
import { BranchNodeView, LeafNodeView } from "base/browser/ui/grid/gridview-view";
...
class BranchNode implements IBranchNode {
	...
	constructor(
		...
		this.tracer = tracer;
		if (tracer) tracer.current_ = this;
...
class LeafNode implements ILeafNode {
	...
	render(): React.ReactNode {
		return React.createElement(LeafNodeView, {node: this});
	}
...
function flipNode<T extends Node>(node: T, size: number, orthogonalSize: number): T {
	if (node instanceof BranchNode) {
		const result = new BranchNode(orthogonal(node.orientation), node.styles, node.proportionalLayout, size, orthogonalSize, node.tracer);
		...
		return new LeafNode((node as LeafNode).view, orthogonal(node.orientation), orthogonalSize, node.tracer) as T;
	}
}

export type Node = BranchNode | LeafNode;

export class GridViewView extends React.Component<{gridView: GridView}> {

	render() {
		const gridView = this.props.gridView;
		return gridView.root.render();
	}
...
export class GridView implements IGridView {
	...
	addView(view: IView, size: number | Sizing, location: number[]): GridNodeTracer {
		const tracer = new GridNodeTracer();
		...
		if (parent instanceof BranchNode) {
			const node = new LeafNode(view, orthogonal(parent.orientation), parent.orthogonalSize, tracer);
			parent.addChild(node, size, index);
		} else {
			const [, grandParent] = tail2(pathToParent);
			const [, parentIndex] = tail2(rest);
			const parentTracer = parent.tracer;
			...
			const newSibling = new LeafNode(parent.view, grandParent.orientation, parent.size, parentTracer);
			...
			const node = new LeafNode(view, grandParent.orientation, parent.size, tracer);
	...
	getNodeLocation(node: Node): number[] {
		if (node == this._root) return [];
		const parent = node.parent;
		if (!(parent instanceof BranchNode))
			throw new Error('Node must be a BranchNode');
		return [...this.getNodeLocation(parent), parent.children.indexOf(node)];
	}
