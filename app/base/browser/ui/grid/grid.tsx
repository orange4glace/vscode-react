...
import { orthogonal, IView, IGridView, Sizing as GridViewSizing, Box, IGridViewStyles } from './gridview';
import { Event, Emitter, filter, any } from 'base/common/event';
import { Split, Distribute, LayoutPriority } from 'base/browser/ui/splitview/splitview';
import { GridView as GridViewModel, GridViewView, GridNodeTracer } from 'base/browser/ui/grid/gridview-impl';
...
export class GridView<T extends IView> extends React.Component<{grid: Grid<T>}> {

	render() {
		const grid = this.props.grid;
		return (
			<GridViewView gridView={grid.gridview}/>
		)
	}
...
export class Grid<T extends IView> implements IDisposable {
	...
	private views = new Map<T, GridNodeTracer>();
	...
	constructor(view: T, options: IGridOptions = {}) {
		this.gridview = new GridViewModel(options);
	...
	protected _addView(newView: T, size: number | GridViewSizing, location: number[]): void {
		const tracer = this.gridview.addView(newView, size, location);
		this.views.set(newView, tracer);
	}
	...
	private getViewLocation(view: T): number[] {
		const tracer = this.views.get(view);

		if (!tracer || !tracer.current) {
			throw new Error('View not found');
		}

		return this.gridview.getNodeLocation(tracer.current);
	}
