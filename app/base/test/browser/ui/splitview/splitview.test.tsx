...
import { SplitView, IView, Orientation, Sizing, LayoutPriority, Distribute, Split } from 'base/browser/ui/splitview/splitview';
...
test('empty splitview has empty DOM', () => {
	...
	const wrapper = mount(<SplitViewView splitView={splitview}/>);
	assert.equal(wrapper.getDOMNode().firstElementChild!.firstElementChild!.childElementCount, 0, 'split view should be empty');
	splitview.dispose();
});
...
test('has views and sashes as children', () => {
	...
	let viewQuery = wrapper.getDOMNode().querySelectorAll('.monaco-split-view2 > .split-view-container > .split-view-view');
	assert.equal(viewQuery.length, 3, 'split view should have 3 views');

	let sashQuery = wrapper.getDOMNode().querySelectorAll('.monaco-split-view2 > .sash-container > .monaco-sash');
	assert.equal(sashQuery.length, 2, 'split view should have 2 sashes');

	splitview.removeView(2);

	viewQuery = wrapper.getDOMNode().querySelectorAll('.monaco-split-view2 > .split-view-container > .split-view-view');
	assert.equal(viewQuery.length, 2, 'split view should have 2 views');

	sashQuery = wrapper.getDOMNode().querySelectorAll('.monaco-split-view2 > .sash-container > .monaco-sash');
...
test('sashes are properly enabled/disabled', () => {
	...
	splitview.addView(view1, Distribute);
	splitview.addView(view2, Distribute);
	splitview.addView(view3, Distribute);
...
test('issue #35497', () => {
	...
	const viewContainers = wrapper.getDOMNode().querySelectorAll('.split-view-view');
...
test('automatic size distribution', () => {
	...
	splitview.removeView(1, Distribute);
...
test('split sizing', () => {
	...
	splitview.addView(view2, Split(0));
	assert.deepEqual([view1.size, view2.size], [100, 100]);

	splitview.addView(view3, Split(1));
...
test('split sizing 2', () => {
	...
	splitview.addView(view3, Split(0));
...
test('disable proportional layout', () => {
	const view1 = new TestView(20, Number.POSITIVE_INFINITY);
	const view2 = new TestView(20, Number.POSITIVE_INFINITY);
	const splitview = new SplitView({ proportionalLayout: false });
	const wrapper = mount(<SplitViewView splitView={splitview}/>);
