...
import { Direction, getRelativeLocation, Orientation, SerializableGrid, ISerializableView, IViewDeserializer, GridNode, Sizing, isGridBranchNode, sanitizeGridNodeDescriptor, GridNodeDescriptor, createSerializedGrid, Grid, GridView } from 'base/browser/ui/grid/grid';
  ...
  render() {
    return (
			<div style={{
				...
				{this.state.grid && <GridView<any> grid={this.state.grid}/>}
			</div>
		)
  }
...
test('empty', () => {
	...
	container.setState({grid: gridview});
...
class TestViewDeserializer implements IViewDeserializer<TestSerializableView> {
	...
	private views: Map<string, TestSerializableView>;

	constructor() {
		this.views = new Map<string, TestSerializableView>();
	}
